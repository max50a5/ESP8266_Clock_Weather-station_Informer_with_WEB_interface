...
void handle_ConfigSetupJson() {
  ...
  json += "\",\"sensorDom\":\"";
  json += (sensorDom==0?"NONE":sensorDom==1?"DS18B20":sensorDom==2?"SI7021":sensorDom==3?"BMP280":sensorDom==4?"BME280":"NONE");
  json += "\",\"corrTempD\":\"";
  json += corrTempD;
  json += "\",\"Td\":\"";
  json += t0;
  json += "\",\"sensorUl\":\"";
  json += (sensorUl==0?"NONE":sensorUl==1?"DS18B20":sensorUl==2?"SI7021":sensorUl==3?"BMP280":sensorUl==4?"BME280":"NONE");
  json += "\",\"corrTempU\":\"";
  json += corrTempU;
  json += "\",\"Tu\":\"";
  json += t3;
  json += "\",\"sensorHome\":\"";
  json += (sensorHome==0?"NONE":sensorHome==1?"DS18B20":sensorHome==2?"SI7021":sensorHome==3?"BMP280":sensorHome==4?"BME280":"NONE");
  json += "\",\"corrTempH\":\"";
  json += corrTempH;
  json += "\",\"Th\":\"";
  json += t6;
...
void handle_setup(){
  ...
  if(printCom) {
    printTime();
    Serial.println("TBD: "+String(timeDay)+", VBD: "+String(volBrightnessD)+", TBN: "+String(timeNight)+", VBN: "+String(volBrightnessN)+",  kuOn: "+String(kuOn)+",  kuOff: "+String(kuOff)+",  rotate0: "+String(rotate0)+", rotate1: "+String(rotate1)+", bigCklock_x2: "+String(bigCklock_x2)+", buzzerOnOffku: "+String(buzzerOnOffku)+", buzzerOnOff: "+String(buzzerOnOff)+", buzzerSet: "+String(buzzerSet)); 
  }
  saveConfig();
  sensors();
  server.send(200, "text/plain", "OK");
